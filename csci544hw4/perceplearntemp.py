    ...
    for i in range(maxiterations):
        ...
        for j in range(len(allfiles)):
            ...
            if 0 >= (posnegfunc * labels[file[2]]):
                for k in features:
                    posnegweights[k], posnegweights['<bias>'] = posnegweights[k] + labels[file[2]] * features[k], \
                                                                    posnegweights['<bias>'] + labels[file[2]]
            if 0 >= (trudecfunc * labels[file[3]]):
                for k in features:
                    trudecweights[k], trudecweights['<bias>'] = trudecweights[k] + labels[file[3]] * features[k], \
